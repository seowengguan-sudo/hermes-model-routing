...
"""
import json, os, re, sys, io, zipfile, hashlib, time, subprocess, threading
  ...
  <div class="header">
    <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 12px;">
      ...
      <div style="display: flex; gap: 12px;">
        ...
        <button onclick="resetServer()" class="btn-secondary" style="padding: 10px 20px; font-size: 14px;" title="Restart the server process — use after updating doc_reader_onefile.py">♻️ Reset Server</button>
      </div>
    </div>
  </div>
...
async function resetServer() {
  if (!confirm('Restart the server now? This picks up any changes made to doc_reader_onefile.py since it was last started. The page will reload automatically in a few seconds.')) {
    return;
  }
  try {
    await fetch('/restart', { method: 'POST' });
  } catch (e) {
    // The connection is expected to drop as the old process exits — this is normal, not an error.
  }
  const statusEl = document.createElement('div');
  statusEl.textContent = 'Restarting server, please wait…';
  statusEl.style.cssText = 'position:fixed;top:20px;left:50%;transform:translateX(-50%);background:#1a2332;color:#fff;padding:12px 24px;border-radius:8px;z-index:9999;font-size:14px;';
  ...
  const tryReload = async () => {
    ...
    } catch (e) { /* server still coming back up, keep waiting */ }
    if (attempts < 20) { setTimeout(tryReload, 500); }
    else { statusEl.textContent = 'Still restarting… you can also refresh manually.'; }
  };
            ...
            if path == "/settings":
                ...
                def _do_restart():
                    ...
                    try:
                        ...
                        pass  # even if the helper fails to launch, still exit below so start_silent.vbs's
                              # own next manual run (or a future restart click) isn't blocked by a stuck port
                    os._exit(0)
