#!/usr/bin/env python3
"""
Model Validation Engine v2 - Robust
Tests each free-tier model across task categories. Resilient to slow/down
providers: short timeouts, parallel batches, provider-level health gating.
Writes results to /opt/data/model_benchmark/results.json
"""
...
def call_model(provider, model_id, prompt, max_tokens=400):
    ...
    try:
        ...
        obj = json.loads(raw)
        if provider == 'huggingface':
            text = obj[0]['generated_text'] if isinstance(obj, list) else obj.get('generated_text', str(obj))
        else:
...
def score_output(task_id, text):
    ...
    if task_id == 'simple_factual':
        ...
        else: s += 2; notes.append("port missing/wrong")
...
# Build task list (provider, model, task_id)
jobs = []
...
def run_job(job):
    ...
    text, lat, err = call_model(prov, mid, BENCHMARKS[tid])
    return prov, mid, strength, ctx, tid, text, lat, err

print(f"Starting validation: {len(jobs)} calls across all providers (parallel, timeout={TIMEOUT}s)")
...
with ThreadPoolExecutor(max_workers=8) as ex:
    ...
    for fut in as_completed(futs):
        ...
        if key not in results:
            results[key] = {'provider': prov, 'model': mid, 'strength': strength,
                            'context': ctx, 'tasks': {}, 'avg_score': 0, 'avg_latency': 0,
                            'status': 'ok'}
        if err:
            results[key]['tasks'][tid] = {'score': 0, 'latency': round(lat, 2), 'error': err[:100]}
            results[key]['status'] = 'partial' if results[key]['status'] == 'ok' else results[key]['status']
            provider_health.setdefault(prov, []).append(False)
        else:
            sc, note = score_output(tid, text)
            results[key]['tasks'][tid] = {'score': sc, 'latency': round(lat, 2), 'note': note}
            provider_health.setdefault(prov, []).append(True)
        done += 1
        if done % 20 == 0:
            print(f"  progress: {done}/{len(jobs)} calls done")

# Finalize averages
for key, rec in results.items():
    scores = [t['score'] for t in rec['tasks'].values() if 'score' in t]
    lats = [t['latency'] for t in rec['tasks'].values() if 'latency' in t and t['latency'] > 0]
    if scores:
        rec['avg_score'] = round(sum(scores)/len(scores), 2)
    if lats:
        rec['avg_latency'] = round(sum(lats)/len(lats), 2)

# Provider health summary
health_summary = {}
for prov, checks in provider_health.items():
    ok = sum(1 for c in checks if c)
    health_summary[prov] = f"{ok}/{len(checks)} calls succeeded"

with open('/opt/data/model_benchmark/results.json', 'w') as f:
    json.dump({'models': results, 'provider_health': health_summary,
               'validated_at': time.strftime('%Y-%m-%d %H:%M')}, f, indent=2)

print("\n=== VALIDATION COMPLETE ===")
print(f"Models tested: {len(results)}")
for prov, h in health_summary.items():
    print(f"  {prov}: {h}")
print(f"\nTop 5 by score:")
ranked = sorted(results.items(), key=lambda x: x[1].get('avg_score',0), reverse=True)
for k, v in ranked[:5]:
    print(f"  {k:50s} score={v.get('avg_score')} lat={v.get('avg_latency')}s")
