...
§
Skill update: Added Pitfall #15 (state.db race condition #68474) to hermes-cron-model-pinning + created references/state-db-race-condition-prevention.md. Updated hermes-runtime-introspection state-db recovery section with corrected findings. Pre-commit hook blocks .db commits; .gitignore excludes all runtime SQLite/logs/caches. 19/19 ad-hoc verification checks passed.